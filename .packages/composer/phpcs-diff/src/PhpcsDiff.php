...
class PhpcsDiff
{
    ...
    public function __construct(array $argv, CLImate $climate)
    {
        ...
        $this->baseBranch = isset($this->argv[1]) ? 'origin/' . str_replace('origin/', '', $this->argv[1]) : 'origin/develop';
    ...
    public function run()
    {
	    ...
	    if (empty($fileDiff) && $this->isFlagSet('-v')) {
	    ...
	    if(!empty($violations['totals']['error'])) {
		    $this->setExitCode(1);
	    }
    }
     ...
     * @return mixed
     */
    protected function runPhpcs(array $files = [], $ruleset = 'phpcs.xml')
    {
        $exec = 'vendor/bin/phpcs';

	    if (is_file( __DIR__ . '/../../../../.vendor/bin/phpcs' )) {
		    $exec = realpath( __DIR__ . '/../../../../.vendor/bin/phpcs' );
	    ...
	    /**
	     * We need -q (quiet mode here).
	     * If somewhere in the phpcs.xml will be set `<arg value="q"/>`,
	     * then extra output from shell_exec will cause `Unable to run phpcs executable` error.
	     */
        return json_decode(
            shell_exec(escapeshellarg($exec) . ' -q --report=json --standard=' . $ruleset . ' ' . implode(' ', $files)),
            true
        );
    }
    ...
    {
        $this->climate->br()->flank(strtoupper('Start of phpcs check'), '#', 10)->br();
        $this->climate->out(implode(PHP_EOL, $output['messages']));
    ...
    protected function getChangedFiles()
    {
        ...
        $output = explode(PHP_EOL, str_replace(["\r\n", "\r", "\n"], PHP_EOL, $output) );
